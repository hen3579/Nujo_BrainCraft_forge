...
public class BuildHudOverlay implements IGuiOverlay {
    public static final ResourceLocation ID = new ResourceLocation(NujoBraincraft.MODID, "build_hud");
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        // 获取淡入淡出透明度
        float alpha = GameModeManager.getFadeAlpha(GameModeState.BUILD);
        if (alpha <= 0f) return;
    ...
    private void drawModeIndicator(GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        Minecraft mc = Minecraft.getInstance();
        String text = "建造模式 (B键切换)";
        int textWidth = mc.font.width(text);
        int x = screenWidth - textWidth - 10;
        int y = 10;
        guiGraphics.drawString(mc.font, text, x, y, 0x55FF55);
    }
}